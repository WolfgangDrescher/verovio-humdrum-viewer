...
function CircularAudioBuffer(slots) {
	slots = slots || 48;  // originally 24, but now this is not enough for page turns
	// number of buffers
	this.slots = slots;
...
function updateProgress(current, total) {
	...
	var millisec = Math.floor(current * 1000 / SAMPLE_RATE + 0.5);
	if (midiPlayer_lastMillisec > millisec) {
		midiPlayer_lastMillisec = 0;
	}
	if (millisec > midiPlayer_lastMillisec) {
		if (midiPlayer_onUpdate != null) midiPlayer_onUpdate(millisec);
	...
	// the WildWestMidi update callback does not seem to be working,
	// so manually placing it here:
	midiUpdate(millisec);
	// otherwise, the note highlighting is not working if the above
	// line of code is not added.
}
...
function play(millisec) {
	...
	// gain.setValueAtTime(0.25, millisec / 1000.0);
	// gain.exponentialRampToValueAtTime(1, millisec / 1000.0 + 1);
